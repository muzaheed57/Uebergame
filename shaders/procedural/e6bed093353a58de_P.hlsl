...
Fragout main( ConnectData IN,
              ...
              uniform sampler2D normalMap2      : register(S2),
...
)
{
   ...
   {
   float4 bumpNormal = float4( lerp( tex2D( normalMap0, IN.detCoord0.yz ), tex2D( normalMap0, IN.detCoord0.xz ), IN.texCoord.z ).ag * 2.0 - 1.0, 0.0, 0.0 ); // DXTnm
   ...
   gbNormal = lerp( gbNormal, IN.viewToTangent[2], min( detailBlend1, IN.detCoord1.w ) );
   ...
   IN.detCoord2.xy += parallaxOffset( normalMap2, IN.detCoord2.xy, negViewTS, detailIdStrengthParallax2.z * detailBlend2 );
