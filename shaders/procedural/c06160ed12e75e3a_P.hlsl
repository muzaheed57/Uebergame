...
)
{
   ...
   {
   float4 bumpNormal = float4( lerp( tex2D( normalMap2, IN.detCoord2.yz ), tex2D( normalMap2, IN.detCoord2.xz ), IN.texCoord.z ).ag * 2.0 - 1.0, 0.0, 0.0 ); // DXTnm
   bumpNormal.z = sqrt( 1.0 - dot( bumpNormal.xy, bumpNormal.xy ) ); // DXTnm
